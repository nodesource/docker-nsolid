...
```bash
NSOLID_VERSION=2.1.1 make build 
```

### Publishing Images

```bash
DOCKER_REGISTRY=username NSOLID_VERSION=2.1.1 make publish
```

### Cleaning up

```bash
NSOLID_VERSION=2.1.1 make clean # removes download directories `nsolid-bundle-*`
NSOLID_VERSION=2.1.1 make cleanall # runs `make clean` and removes all docker images with label=nodesource=nsolid
```
