...
```bash
NSOLID_VERSION=3.1.1 make alpine
```

#### Debian / Ubuntu

```bash
NSOLID_VERSION=3.1.1 make build
```

### Publishing Images

```bash
DOCKER_REGISTRY=username NSOLID_VERSION=3.1.1 make publish
```

#### Alpine

```bash
DOCKER_REGISTRY=username NSOLID_VERSION=3.1.1 make publish-alpine
```
...
```bash
NSOLID_VERSION=3.1.1 make clean # removes download directories `nsolid-bundle-*`
NSOLID_VERSION=3.1.1 make cleanall # runs `make clean` and removes all docker images with label=nodesource=nsolid
```
