...
```bash
NSOLID_VERSION=3.4.3 make alpine
```

#### Debian / Ubuntu

```bash
NSOLID_VERSION=3.4.3 make build
```

### Publishing Images

```bash
DOCKER_REGISTRY=username NSOLID_VERSION=3.4.3 make publish
```

#### Alpine

```bash
DOCKER_REGISTRY=username NSOLID_VERSION=3.4.3 make publish-alpine
```
...
```bash
NSOLID_VERSION=3.4.3 make clean # removes download directories `nsolid-bundle-*`
NSOLID_VERSION=3.4.3 make cleanall # runs `make clean` and removes all docker images with label=nodesource=nsolid
```
